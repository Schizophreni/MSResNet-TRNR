...
class ImgLIPNfreqsKshot:
    ...
    def _tuple_trans(self, clean_img, noise_sigma):
        if isinstance(noise_sigma, int):
            sigma = noise_sigma
        else:
            sigma = np.random.choice(noise_sigma)
        ...
        noise_img = np.random.randn(*clean_img.shape)*sigma/255.0 + clean_img
    ...
    def load_data_cache(self, dset, mode='train'):
        ...
        for episode in range(20):
            ...
            for batch in range(self.batch_size): ## batch_size -> task num each episode
                ...
                ### noise_sigma = np.random.choice([15, 20, 25])
                ...
                for label, freq in enumerate(selected_freqs[:-1]):
                    ...
                    for index, img in enumerate(selected_imgs[:self.k_shot]):
                        spt_y[label*self.k_shot+index, ...], spt_x[label*self.k_shot+index, ...]  = self._tuple_trans(clean_img=img, noise_sigma=noise_sigma)
                        # spt_x[label*self.k_shot+index, ...] = self.transformation(img.replace('clean', 'rain'))
                    if label < self.n_freqs-1:
                        for index, img in enumerate(selected_imgs[self.k_shot:]):
                            qry_y[label*self.k_query+index, ...],qry_x[label*self.k_query+index, ...] = self._tuple_trans(clean_img=img, noise_sigma=noise_sigma)
...
def main():
    ...
    root_dir = '/home/wran/Public/datasets/denoise/process/BSD500-100/ProcessData/BSD500-100-multi'
    bsz = 4
    n_freqs = 5
    k_shot = 1
    k_query = 1
